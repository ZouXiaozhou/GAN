...
class DCGAN(object):
    def __init__(self, sess, g_iters=250000, input_height=108, input_width=108, is_crop=True,
                 ...
                 learning_rate=0.00005, upper_threshold=0.85, lower_threshold=0.15, logfile="logfile"):
        """
        Args:
        ...
        self.noise_scale = 1
        self.noise_exp = 0.55
        self.noise_interval = 1
        self.noise_come = 1000
        self.noise_step = 10
    ...
    def build_model(self):
        ...
        add_noise_indicator = tf.placeholder(tf.bool, shape=[], name="add_noise_indicator")
        ...
        g_grad_var = g_optimizer.compute_gradients(g_loss
                                                   + self.g_penalty * g_l2norm
                                                   , var_list=g_vars)

        gradient_scale = sum([np.prod(gv[0].get_shape().as_list()) for gv in g_grad_var])

        gradient_absavg = tf.div(tf.reduce_sum([tf.reduce_sum(tf.abs(gv[0])) for gv in g_grad_var]), gradient_scale)

        transformed_g_grad_var = [(self.grad_transform(gv[0], gradient_absavg, add_noise_indicator), gv[1])
                                  for gv in g_grad_var]

        new_gradient_scale = sum([np.prod(gv[0].get_shape().as_list()) for gv in transformed_g_grad_var])

        new_gradient_absavg = tf.div(tf.reduce_sum([tf.reduce_sum(tf.abs(gv[0])) for gv in transformed_g_grad_var]),
                                     new_gradient_scale)

        update_g_var = g_optimizer.apply_gradients(transformed_g_grad_var)
        ...
        return z, add_noise_indicator, inputs, g_loss, gradient_absavg, \
               new_gradient_absavg, d_loss, update_g_var, d_optim, g_sum, d_sum

    def grad_transform(self, gradient, gradient_sum, noise_indicator):

        return tf.cond(
            tf.logical_and(tf.less(gradient_sum, tf.constant(float(self.threshold))), noise_indicator),
                       lambda: tf.add(gradient, tf.abs(
                           tf.truncated_normal(gradient.get_shape(), stddev=0.0001/self.noise_scale))),
                       lambda: tf.identity(gradient))

    def train(self):
        """Train DCGAN"""
        [z, add_noise_indicator, inputs, g_loss, gradient_absavg, new_gradient_absavg,
         d_loss, g_optim, d_optim, g_sum, d_sum] = self.build_model()
        ...
        writer = tf.summary.FileWriter("./logs_an_c1000_r3", self.sess.graph)
        ...
        for g_subiters in range(self.g_iters):
            ...
            self.noise_scale = pow(int(g_subiters / self.noise_step) + 1, self.noise_exp)
            ...
            for j in range(d_iters):
                ...
                self.logfile.write("g_iters: [%2d] discriminator training [%4d/%4d] time: %4.4f, d_loss: %.8f\n" %
                                   (g_subiters, j+1, d_iters, time.time() - start_time, errD))
                batch_images, batch_z, idx = self.get_next_batch_data(idx, batch_idxs, data)

            # Update G network
            for i in range(int(g_subiters * 25 / self.g_iters) + 1):
                self.add_noise = False
                if g_subiters > self.noise_come and np.mod(g_subiters, self.noise_interval) == 0:
                    self.add_noise = True
                _, absavg_gradient, absavg_new_gradient, summary_str = \
                    self.sess.run([g_optim, gradient_absavg, new_gradient_absavg, g_sum],
                                  feed_dict={z: batch_z, add_noise_indicator: self.add_noise})
                writer.add_summary(summary_str, counter)
                batch_images, batch_z, idx = self.get_next_batch_data(idx, batch_idxs, data)
                errD = d_loss.eval({z: batch_z, inputs: batch_images})
                errG = g_loss.eval({z: batch_z})
                self.logfile.write("g_iters: [%2d] [%4d/%4d] time: %4.4f, d_loss: %.8f, g_loss: %.8f, absavg_gradient: %.8f, absavg_new_gradient: %.8f\n" %
                                   (g_subiters, i+1, int(g_subiters * 25 / self.g_iters) + 1,
                                    time.time() - start_time, errD, errG, absavg_gradient, absavg_new_gradient))
                if absavg_gradient < self.threshold and self.add_noise:
                    self.logfile.write("gradient noise added \n")
    ...
    def get_next_batch_data(self, idx, batch_idxs, data):
        ...
        if batch_images.shape != (64, 64, 64, 3):
            print (batch_images, idx, batch_idxs)
    ...
    def discriminator(self, image, reuse=False):
        with tf.variable_scope("discriminator") as scope:
            ...
            return linear(tf.reshape(h3, [self.batch_size, -1]), 1, 'd_h3_lin')
